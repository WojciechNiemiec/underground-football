...
class PlayerPaymentService {
    ...
    PlayerPaymentService(PaymentSagaRepository paymentSagaRepository, GameAccessor gameAccessor, WalletAccessor walletAccessor) {
    ...
    void handle(ChargePaid event) {
        PaymentSaga saga = paymentSagaRepository.load(event.transactionId());
    ...
    private void endSaga(UUID gameId, UUID playerId) {
        ...
        paymentSagaRepository.delete(transactionId);
    }
